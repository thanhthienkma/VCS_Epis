...
class WordPress {
  ...
  static Future fetchItems() {
    String url = '${Config.apiEndpoint}/posts';
    return http.get(url);
  }
  ...
  static Future fetchCategories() {
    String url =
        '${Config.apiEndpoint}/categories?per_page=100&orderby=id&order=asc';
    if (Config.excludeCategories.length > 0) {
      url += '&exclude=${Config.excludeCategories.join(',')}';
    }
    print("AAAAAAAAAAAAAAA" + url);
    return http.get(url);
  }
  ...
  /// Fetch 10 posts
  static Future fetchPostsByNews() {
    String url = 'https://vcs-vn.com.au/wp-json/vcsvn/v1/latest-posts/31';
    return http.get(url);
  }
  ...
  static Future fetchPostsFromList(List<int> posts) {
    String url =
        '${Config.apiEndpoint}/posts?_embed&include=${posts.join(',')}';
    return http.get(url);
  }
  ...
  static Future fetchComments(int postID, {int page = 1, int perPage = 10}) {
    final String url =
        '${Config.apiEndpoint}/comments?post=$postID&page=$page&per_page=$perPage';
    return http.get(url);
  }
