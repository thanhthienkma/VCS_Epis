...
class DistrictWidget extends StatefulWidget {
  ...
  DistrictWidget({this.callback});

  @override
  State<StatefulWidget> createState() => _DistrictWidgetState();
}
...
class _DistrictWidgetState extends State<DistrictWidget> {
    ...
    return Container(
      ...
      child: Stack(
        children: <Widget>[
          ...
          _districtListWidget((String value) {
            /// Search district
  ...
  Widget _districtListWidget(Function(String value) callback) {
    return Container(
      height: 300,
  ...
  void searchDistrict(String value) {
    if (value.isEmpty) {
      setState(() {
        searchList = list;
      });
      ...
      searchList = list.where((element) {
        ...
        if (districtName.toLowerCase().contains(value.toLowerCase())) {
          return true;
        } else {
          return false;
        }
  ...
  getDistricts() async {
    try {
      Response response =
          await Dio().get('https://vanchuyen.etado.vn/api/v1/districts?');
      print(response);
