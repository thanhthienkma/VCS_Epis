...
class ProvinceWidget extends StatefulWidget {
  ...
  ProvinceWidget({this.callback});

  @override
  State<StatefulWidget> createState() => _ProvinceWidgetState();
}
...
class _ProvinceWidgetState extends State<ProvinceWidget> {
    ...
    return Container(
      ...
      child: Stack(
        children: <Widget>[
          ...
          _provinceListWidget((String value) {
  ...
  Widget _provinceListWidget(Function(String value) callback) {
    return Container(
      height: 300,
  ...
  void searchProvince(String value) {
    if (value.isEmpty) {
      setState(() {
        searchList = list;
      });
      ...
      searchList = list.where((element) {
        ...
        if (provinceName.toLowerCase().contains(value.toLowerCase())) {
          return true;
        } else {
          return false;
        }
  ...
  getProvinces() async {
    try {
      Response response =
          await Dio().get('https://vanchuyen.etado.vn/api/v1/provinces?');
      print(response);
